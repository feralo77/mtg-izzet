...
2. El orquestador crea en Drive (carpeta "MTG · Izzet") la subcarpeta **`Logs_<nick>`** con la
   hoja **"Partidas — <nick>"** dentro (columnas: Fecha, Evento / Liga, Ronda, Lista, Notas,
   Mazo rival opcional), y registra el nick en [`../automation/jugadores.json`](../automation/jugadores.json)
...
2. **Apuntar 3 celdas por partida** en su hoja "Partidas — <nick>": Fecha, Liga y Ronda.
   El resto (resultado, marcador, rival, mazo detectado, salida/robo, mulligans) lo saca el
   robot de los logs y lo empareja por fecha + orden de partida.
   - "Mazo rival": solo si ese día jugó partidas sueltas además de la Liga (desambigua).
